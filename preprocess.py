...
def del_small_con(roicon):
    ...
    for _ in range(tmp_l):
        ...
        if (tmp_roicon[2] <= 3 and tmp_roicon[3] <= 3) or (tmp_roicon[2] <= 3 and tmp_roicon[3] <= 3):
            del roicon[j]
            i += 1
            continue
        if i + 1 > tmp_l - 1:
...
def process(srcimg):
    ...
    if flag == False:
        ...
        for split_line in split_line_list:
            ...
            # print('contour_row:', contour_row)
            distance = np.array(
                [contour_row[i + 1][0] - contour_row[i][0] - contour_row[i][2] for i in range(len(contour_row) - 1)])
...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        srcimg = cv2.imread(path, 0)
        roi, roicon = process(srcimg)   # [row_up, row_down], [(), (), ...()]
